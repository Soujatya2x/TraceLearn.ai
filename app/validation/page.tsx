...
import {
  ...
  Loader2, ChevronDown, XCircle, Shield, Lightbulb, MessageSquare,
} from 'lucide-react'
...
// ─── Mock data — shown when backend hasn't returned fix validation yet ────────
// Real data: GET /api/v1/session/{id}/analysis + /attempts
// Mapped via mapSessionToValidationResult() in analysisService.ts
// fixedCode, whatChanged, whyItWorks come from AI agent POST /ai/analyze response.
...
export default function ValidationPage() {
  ...
  // ── Real data: GET /api/v1/session/{id} + /attempts ───────────────────────
  // mapSessionToValidationResult() assembles this from BackendSession + attempts.
  // fixedCode & explanation come from session.aiAnalysis (AI agent response).
  const validationQuery = useValidationResult(currentSessionId)
  const { data: d, isPreview, isLoading } = useFallback(validationQuery, MOCK_VALIDATION)
                ...
                <div className="flex flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex items-center gap-2">
                    ...
                    <div className="flex items-center gap-2 pl-2 border-l border-border/50">
                      ...
                      <motion.button
                        onClick={handleRetry}
                        disabled={retryMutation.isPending || d.retryCount >= d.maxRetries || isPreview}
