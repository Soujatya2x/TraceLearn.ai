...
import {
  BarChart3, BookOpen, TrendingUp, Flame, FolderOpen,
} from 'lucide-react'
  ...
  useEffect(() => {
    ...
    const timeout = setTimeout(() => {
      ...
      const iv = setInterval(() => {
        current += step
        if (current >= to) { setValue(to); clearInterval(iv) }
...
export default function ArtifactsPage() {
  ...
  const { data: artifactsData, isLoading } = useArtifacts(currentSessionId)
  ...
  const displayData = artifactsData ?? MOCK_ARTIFACTS
  ...
  // Detect when filter bar reaches the top so we can add a shadow
  useEffect(() => {
    const el = filterRef.current
    if (!el) return
    const observer = new IntersectionObserver(
      ([entry]) => setFilterStuck(!entry.isIntersecting),
      { threshold: 1, rootMargin: '-57px 0px 0px 0px' }, // 56px navbar
    )
    observer.observe(el)
  ...
  return (
    <AppShell activeNav="artifacts">
      <div className="px-6 py-8 max-w-6xl mx-auto">
        ...
        {/* ── Filter bar — sticky with shadow when stuck ────────────────── */}
        <div ref={filterRef} className="sticky top-14 z-10 mb-5 -mx-6 px-6 py-3 transition-all duration-200"
          style={{
