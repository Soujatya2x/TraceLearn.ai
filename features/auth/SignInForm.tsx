...
import { Mail, Lock, Eye, EyeOff, Github, ArrowRight, AlertCircle } from 'lucide-react'
...
function AuthInput({ id, label, type, value, onChange, placeholder, autoComplete, icon: Icon, suffix }: {
    ...
    <div className="space-y-1.5">
      ...
      <div className="relative">
        <Icon className="absolute left-3 top-1/2 -translate-y-1/2 w-3.5 h-3.5 pointer-events-none"
          style={{ color: focused ? '#06b6d4' : 'rgba(255,255,255,0.3)' }} />
        <input id={id} type={type} value={value}
          ...
          style={{
            ...
            border: `1px solid ${focused ? 'rgba(79,70,229,0.6)' : 'rgba(255,255,255,0.09)'}`,
...
export function SignInForm({ next, onSuccess }: Props) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try { await signInEmail({ email, password }); onSuccess() } catch {}
  }
    ...
    <div className="space-y-3.5">
      ...
      <form onSubmit={handleSubmit} className="space-y-3">
        ...
        <div className="flex justify-end">
          <Link href="/auth/forgot-password" style={{ color: '#06b6d4', fontSize: 12 }} className="hover:opacity-80 transition-opacity">
            Forgot password?
          </Link>
        </div>
        <motion.button type="submit" disabled={isLoading || !email || !password}
          whileTap={{ scale: 0.98 }} whileHover={email && password ? { scale: 1.01 } : undefined}
          className="w-full flex items-center justify-center gap-2 py-2.5 rounded-xl text-sm font-semibold"
          style={{
            background: email && password ? 'linear-gradient(135deg, #4f46e5, #06b6d4)' : 'rgba(79,70,229,0.35)',
            color: '#fff',
            boxShadow: email && password ? '0 0 24px rgba(79,70,229,0.35)' : 'none',
